...
public static class FBPayments
{
    ...
    //
    private static readonly string PaymentObjectURL = GameSettings.ServerURL + "payments/{0}.php";
    private static readonly Dictionary<int,string> PaymentObjects = new Dictionary<int, string>
    {
        { 0, "100coins" },
        { 1, "250coins" },
        { 2, "800coins" }
    ...
    {
        // Format payment URL
        string paymentURL = string.Format(PaymentObjectURL, PaymentObjects[(int)cPackage]);
                      ...
                      (IPayResult result) =>
        {
            ...
            if (result.ResultDictionary.TryGetValue("payment_id", out payIdObj))
            {
                ...
                {
					GameSettings.CoinBalance += (int)cPackage;
