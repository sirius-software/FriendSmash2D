...
public static class FBRequest
{
    ...
    public static void RequestChallenge ()
    {
        ...
        if (GameSettings.Score != 0 && GameSettings.FriendID != null)
        {
            // We have played a game -- lets send a request to the person we just smashed
            title = "Friend Smash Challenge!";
            message = "I just smashed you " + GameSettings.Score.ToString() + " times! Can you beat it?";
            recipient = new List<string>(){ GameSettings.FriendID };
            data = "{\"challenge_score\":" + GameSettings.Score.ToString() + "}";
        }
        else
        {
